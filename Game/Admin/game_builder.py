...
class GameBuilder:
    ...
    @classmethod
    def config2singlestate(cls, player, config):
        ...
        """
        board = cls.board_from_config(player, config)
        if board:
    ...
    @classmethod
    def state_from_config(cls, players, game_config):
        ...
        try:
            ...
            state = cls.build(BuildType.STATE_CONFIG, state_type, [players, game_config["config"]])
            if state:
                return state
    ...
    def board_from_config(cls, players, game_config):
        ...
        try:
            ...
            board = cls.build(BuildType.BOARD_CONFIG, board_type, [players, game_config["config"]])
            if board:
                return board
        except Exception as e:
            print(e)
    ...
    def state_from_info(cls, state_info):
        ...
        try:
            ...
            state = cls.build(BuildType.STATE_INFO, state_type, [state_info["info"]])
            if state:
                return state
        except Exception as e:
            print(e)
    ...
    def board_from_info(cls, board_info):
        ...
        try:
            ...
            board = cls.build(BuildType.BOARD_INFO, board_type, [board_info["info"]])
            if board:
                return board
        except Exception as e:
            print(e)
