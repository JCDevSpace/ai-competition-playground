...
class Referee:
    ...
    #       if uniform is True, then uniform_fish_num must be specified as a number >= 1 and min_holes, min_one_fish and specific_holes must not be specified
    #       if specified uniform_fish_num must be in the range [1, 5]
    #       the total number tiles (rows * cols) must be greater than the number of required tiles (min_one_fish + max(min_holes, len(specific_holes))
    ...
    # This means the board will have a random number of fish everywhere and at least min_one_fish tiles with 1 fish on it
    # Int -> Void
    ...
    # It does this by adding holes where they are specified, and then add random holes untill we have enough.
    # List[Posn], Int -> void
